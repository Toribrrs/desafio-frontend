...
import { SunIcon, MoonIcon, UserCircleIcon, BellIcon, ArrowPathIcon, LockClosedIcon } from '@heroicons/react/24/outline';

export default function Settings() {
  const { theme, toggleTheme } = useContext(ThemeContext);

  const cardBgColor = theme === 'dark' ? 'bg-slate-800' : 'bg-white';
  const cardBorderColor = theme === 'dark' ? 'dark:border-slate-700' : 'border-gray-200';
    ...
    <div className="p-6 max-w-3xl mx-auto transition-colors duration-300">
      ...
      <section className={`${cardBgColor} p-6 rounded-2xl shadow-md border ${cardBorderColor} mb-6`}>
        <div className="flex items-center space-x-4 mb-5">
          ...
          <div>
            ...
            <p className={`text-sm ${textColor}`}>Edite seu nome, e-mail e outras informações pessoais.</p>
          </div>
        </div>
        <div className="space-y-4">
          <div className="flex flex-col">
            <label htmlFor="name" className="text-sm font-medium text-gray-900 dark:text-gray-300">Nome Completo</label>
            <input
              type="text"
              id="name"
              placeholder="Seu nome"
              className={`mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-600 ${inputBgColor} px-4 py-2 ${inputTextColor} focus:ring-teal-500 focus:border-teal-500 transition-colors`}
            />
          </div>
          <div className="flex flex-col">
            <label htmlFor="email" className="text-sm font-medium text-gray-900 dark:text-gray-200">Endereço de E-mail</label>
            <input
              type="email"
              id="email"
      ...
      <section className={`${cardBgColor} p-6 rounded-2xl shadow-md border ${cardBorderColor} mb-6`}>
        <div className="flex items-center space-x-4 mb-5">
          ...
          <div>
            ...
            <p className={`text-sm ${textColor}`}>Personalize a interface do usuário com um tema claro ou escuro.</p>
          </div>
        </div>
        <div className="flex space-x-4 p-1 bg-gray-100 dark:bg-gray-700 rounded-xl">
          <button
            onClick={() => toggleTheme('light')}
            className={`flex-1 p-3 flex items-center justify-center space-x-2 rounded-lg transition-all duration-200 ${
              theme === 'light' ? 'bg-white shadow text-teal-600' : 'text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`flex-1 p-3 flex items-center justify-center space-x-2 rounded-lg transition-all duration-200 ${
              theme === 'dark' ? 'bg-gray-200 shadow text-purple-600' : 'text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white'
            }`}
          >
      ...
      <section className={`${cardBgColor} p-6 rounded-2xl shadow-md border ${cardBorderColor} mb-6`}>
        ...
        <div className="space-y-4">
          <label htmlFor="email-notifications" className="flex items-start gap-3 cursor-pointer">
            ...
            <div>
              <span className="font-medium text-gray-900 dark:text-gray-100">Notificações por e-mail</span>
              <p className="text-sm text-gray-600 dark:text-gray-400">Receba atualizações importantes, novidades e alertas de segurança por e-mail.</p>
            </div>
          </label>
          <label htmlFor="auto-updates" className="flex items-start gap-3 cursor-pointer">
            ...
            <div>
              <span className="font-medium text-gray-900 dark:text-gray-100">Atualizações automáticas</span>
              <p className="text-sm text-gray-600 dark:text-gray-400">Mantenha a aplicação sempre na versão mais recente com atualizações automáticas.</p>
            </div>
          </label>
        </div>
      </section>
      ...
      <section className={`${cardBgColor} p-6 rounded-2xl shadow-md border ${cardBorderColor}`}>
        ...
        <div className="space-y-3 sm:space-y-0 sm:flex sm:space-x-4">
          <button className="w-full sm:w-auto px-6 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 transition">
            Alterar Senha
          </button>
          <button className="w-full sm:w-auto px-6 py-2 border border-red-500 text-red-500 rounded-lg hover:bg-red-50 dark:hover:bg-red-900 transition">
            Desativar Conta
          </button>
        </div>
      </section>
